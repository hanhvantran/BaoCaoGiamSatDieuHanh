...
export default class SoVoiKeHoachScreen extends React.PureComponent {
  static navigationOptions = {
    title: "Thu tiền điện so với kế hoạch giao"
  };
  constructor(props) {
    super(props);
    this.state = {
  ...
  callMultiAPI = async (vThangNam, vMaDonVi) => {
    ...
    const urls = [urlBaoCao.GET_TiLeThuTienDien + param1];
    Promise.all(
      urls.map(url =>
        fetch(url)
