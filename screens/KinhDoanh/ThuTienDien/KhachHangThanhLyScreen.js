...
export default class KhachHangThanhLyScreen extends React.PureComponent {
  ...
  callMultiAPI = async (vThangNam, vMaDonVi) => {
    ...
    const urls = [urlBaoCao.GET_KHangThanhLyNoKhoDoi + param1];
    Promise.all(
      urls.map(url =>
        fetch(url)
  ...
  render() {
    ...
    let vNoKhoDoiThang = 0;
    let vKhoanhNoThang = 0;
    let vThanhLyThang = 0;
    let vNoKhoDoiLuyKe = 0;
    let vKhoanhNoLuyKe = 0;
    let vThanhLyLuyKe = 0;
    let list1 = [];
    let varCategories1 = [];
    if (
      ...
      for (let i = 0; i < this.state.listDaTa.Series[0].data.length; i++) {
        vNoKhoDoiThang = vNoKhoDoiThang + this.state.listDaTa.Series[0].data[i];
      }
      for (let i = 0; i < this.state.listDaTa.Series[1].data.length; i++) {
        vKhoanhNoThang = vKhoanhNoThang + this.state.listDaTa.Series[1].data[i];
      }
      for (let i = 0; i < this.state.listDaTa.Series[2].data.length; i++) {
        vThanhLyThang = vThanhLyThang + this.state.listDaTa.Series[2].data[i];
      }
      for (let i = 0; i < this.state.listDaTa.Series[3].data.length; i++) {
        vNoKhoDoiLuyKe = vNoKhoDoiLuyKe + this.state.listDaTa.Series[3].data[i];
      }
      for (let i = 0; i < this.state.listDaTa.Series[4].data.length; i++) {
        vKhoanhNoLuyKe = vKhoanhNoLuyKe + this.state.listDaTa.Series[4].data[i];
      }
      for (let i = 0; i < this.state.listDaTa.Series[5].data.length; i++) {
        vThanhLyLuyKe = vThanhLyLuyKe + this.state.listDaTa.Series[5].data[i];
      }
    }
    list1.push(vNoKhoDoiThang);
    list1.push(vKhoanhNoThang);
    list1.push(vThanhLyThang);
    list1.push(vNoKhoDoiLuyKe);
    list1.push(vKhoanhNoLuyKe);
    list1.push(vThanhLyLuyKe);
    varCategories1.push("Nợ khó đòi tháng");
    varCategories1.push("Khoanh nợ tháng");
    varCategories1.push("Thanh lý tháng");
    varCategories1.push("Nợ khó đòi luỹ kế");
    varCategories1.push("Khoanh nợ luỹ kế");
    varCategories1.push("Thanh lý luỹ kế");
    const options = {
      global: {
        useUTC: false
      },
    ...
    var conf2 = {
      ...
      title: {
        text: "Khách hàng thanh lý theo đơn vị"
      },
      yAxis: {
        title: {
          text: "Số lượng"
