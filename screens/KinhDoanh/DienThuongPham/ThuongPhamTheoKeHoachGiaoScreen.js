...
export default class ThuongPhamTheoKeHoachGiaoScreen extends React.PureComponent {
  static navigationOptions = {
    title: "Thương phẩm theo kế hoạch giao"
  };
  constructor(props) {
    super(props);
    this.state = {
  ...
  callMultiAPI = async (vThangNam, vMaDonVi) => {
    ...
    const urls = [urlBaoCao.GET_ThuongPhamSoVoiKeHoach + param1];
    Promise.all(
      urls.map(url =>
        fetch(url)
           ...
           
 {
  ...
  render() {
    ...
    let colum1 = Number(
      vTong3 == 0 ? 0.0 : ((vTong1 * 100) / vTong3).toFixed(2)
    );
    let colum2 = Number(
      vTong3 == 0 ? 0.0 : ((vTong2 * 100) / vTong3).toFixed(2)
    );
    let colum3 = 100;
    list2.push(colum1);
    list2.push(colum2);
    ...
    list1.push(vTong2);
    list1.push(vTong3);
    varCategories1.push("Sản lượng tháng");
    varCategories1.push("Sản lượng luỹ kế");
    ...
    var conf2 = {
      ...
      title: {
        text: "Tỉ lệ so với kế hoạch giao"
      },
      yAxis: {
        title: {
          text: "%"
