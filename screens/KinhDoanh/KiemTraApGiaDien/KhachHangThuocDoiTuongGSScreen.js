...
export default class KhachHangThuocDoiTuongGSScreen extends React.PureComponent {
  static navigationOptions = {
    title: "Khách hàng thuộc đối tượng giám sát"
  };
  constructor(props) {
    super(props);
    this.state = {
            ...
            
 {
  ...
  render() {
    ...
    var conf3 = {
      ...
      title: {
        text: "Khách hàng thuộc đối tượng giám sát theo đơn vị"
      },
      yAxis: {
        title: {
          text: "Số lượng"
    ...
    if (this.state.listDaTa && !Array.isArray(this.state.listDaTa) && this.state.listDaTa.Series!=null) {
      ...
      for (let i = 0; i < this.state.listDaTa.Series[5].data.length; i++) {
        vTong6 = vTong6 + this.state.listDaTa.Series[5].data[i];
      }
      for (let i = 0; i < this.state.listDaTa.Series[6].data.length; i++) {
        vTong7 = vTong7 + this.state.listDaTa.Series[6].data[i];
      }
      for (let i = 0; i < this.state.listDaTa.Series[7].data.length; i++) {
        vTong8 = vTong8 + this.state.listDaTa.Series[7].data[i];
      }
  ...
  //  list1.push(vTong9);
    ...
    varCategories1.push("SANXUAT");
    varCategories1.push("KDDV");
    varCategories1.push("Giá khác");
    /*
    varCategories1.push("COTILEGIA");
    varCategories1.push("KHONGCOTILEGIA");
    varCategories1.push("SHBTD");
*/
    var conf1 = {
      chart: {
        type: "column",
