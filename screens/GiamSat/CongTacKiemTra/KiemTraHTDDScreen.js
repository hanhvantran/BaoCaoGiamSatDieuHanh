...
export default class KiemTraHTDDScreen extends React.PureComponent {
  ...
  callMultiAPI = async (vThangNam, vMaDonVi) => {
    ...
    const urls = [urlBaoCao.SP_KTraHTDD + param1];
    Promise.all(
      urls.map(url =>
        fetch(url)
  ...
  render() {
    ...
    if (
      ...
      let intViPham = 0;
      for (let i = 0; i < this.state.listDaTa1.Series[0].data.length; i++) {
        intViPham = intViPham + this.state.listDaTa1.Series[0].data[i];
      }
      let intViPhamHC = 0;
      for (let i = 0; i < this.state.listDaTa1.Series[1].data.length; i++) {
        intViPhamHC = intViPhamHC + this.state.listDaTa1.Series[1].data[i];
      }
      let intKhongViPham = 0;
      for (let i = 0; i < this.state.listDaTa1.Series[2].data.length; i++) {
        intKhongViPham =
          intKhongViPham + this.state.listDaTa1.Series[2].data[i];
      }
      let intTong = 0;
      for (let i = 0; i < this.state.listDaTa1.Series[3].data.length; i++) {
        intTong = intTong + this.state.listDaTa1.Series[3].data[i];
      }
      ...
      for (let i = 0; i < this.state.listDaTa1.Series[6].data.length; i++) {
        intKhongViPhamLuyKe =
          intKhongViPhamLuyKe + this.state.listDaTa1.Series[6].data[i];
      }
      let intTongLuyKe = 0;
      for (let i = 0; i < this.state.listDaTa1.Series[7].data.length; i++) {
        intTongLuyKe = intTongLuyKe + this.state.listDaTa1.Series[7].data[i];
      }
      varCategories1.push("Vi phạm sử dụng điện");
      varCategories1.push("Vi phạm hành chính");
      varCategories1.push("Không vi phạm");
      varCategories1.push("Tổng");

      list1.push(intViPham);
      list1.push(intViPhamHC);
      list1.push(intKhongViPham);
      list1.push(intTong);

      list2.push(intViPhamLuyKe);
      list2.push(intViPhamHCLuyKe);
      list2.push(intKhongViPhamLuyKe);
      list2.push(intTongLuyKe);
    }
    ...
    var conf2 = {
      ...
      title: {
        text: "Số lượt kiểm tra HTĐĐ theo đơn vị"
      },
      yAxis: {
        title: {
          text: "Số lượt"
